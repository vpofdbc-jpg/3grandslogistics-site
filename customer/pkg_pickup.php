...
$sets=[]; if($has('pickup_requested_at')) $sets[]="pickup_requested_at=NOW()";
if($has('ready_at')) $sets[]="ready_at=NOW()";
if($has('status')) $sets[]="status='Ready for Pickup'";
$sql="UPDATE packages SET ".implode(',', $sets)." WHERE id=? LIMIT 1";
$u=$conn->prepare($sql); $u->bind_param('i',$pkgId); $u->execute(); $u->close();

header('Location: /customer/dashboard.php?ok=pickup_requested');
